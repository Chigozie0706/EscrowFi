...
const statusConfig = {
  ...
  funded: { label: "Funded", color: "bg-[#14F195]/10 text-[#14F195] border-[#14F195]/20" },
  accepted: { label: "In Progress", color: "bg-[#7C3AED]/10 text-[#7C3AED] border-[#7C3AED]/20" },
  submitted: { label: "Submitted", color: "bg-yellow-500/10 text-yellow-500 border-yellow-500/20" },
  released: { label: "Completed", color: "bg-[#14F195]/10 text-[#14F195] border-[#14F195]/20" },
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">Welcome back! Here's what's happening with your escrows.</p>
        </div>
            ...
            <Card className="border-border/40 bg-card/50 backdrop-blur-sm hover:border-[#7C3AED]/30 transition-all duration-300">
              <CardContent className="p-6">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className={`flex items-center gap-1 text-sm ${stat.trend === 'up' ? 'text-[#14F195]' : 'text-red-500'}`}>
                    ...
                    <div className="p-4 rounded-xl border border-border/40 hover:border-[#7C3AED]/50 transition-all duration-300 hover:bg-accent/50 group cursor-pointer">
                      <div className="flex items-start justify-between mb-3">
                        <div className="flex-1">
                          <h4 className="font-semibold mb-1 group-hover:text-[#7C3AED] transition-colors">{escrow.title}</h4>
                          <div className="flex items-center gap-4 text-sm text-muted-foreground">
                            <span>Client: {escrow.client}</span>
                            {escrow.freelancer && <span>Freelancer: {escrow.freelancer}</span>}
                          </div>
                        </div>
                        <Badge variant="outline" className={statusConfig[escrow.status as keyof typeof statusConfig].color}>
                          {statusConfig[escrow.status as keyof typeof statusConfig].label}
                        </Badge>
                      </div>
                      
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-sm text-muted-foreground">Progress</span>
                        <span className="text-sm font-medium">{escrow.progress}%</span>
                      </div>
                      ...
                      <div className="flex items-center justify-between mt-3 pt-3 border-t border-border/40">
                        ...
                        <span className="text-sm text-muted-foreground">Due: {escrow.deadline}</span>
                      </div>
                    </div>
                  </Link>
                </motion.div>
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">≈ $2,489.00 USD</p>
                </div>
                ...
                {recentActivity.map((activity, index) => (
                  ...
                  >
                    <div className={`w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0 ${
                      activity.type === 'released' ? 'bg-[#14F195]/10' :
                      activity.type === 'created' ? 'bg-blue-500/10' :
                      activity.type === 'submitted' ? 'bg-yellow-500/10' :
                      'bg-[#7C3AED]/10'
                    }`}>
                      {activity.type === 'released' && <CheckCircle2 className="w-5 h-5 text-[#14F195]" />}
                      {activity.type === 'created' && <DollarSign className="w-5 h-5 text-blue-500" />}
                      {activity.type === 'submitted' && <Clock className="w-5 h-5 text-yellow-500" />}
                      {activity.type === 'accepted' && <Briefcase className="w-5 h-5 text-[#7C3AED]" />}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium mb-1">{activity.title}</p>
                      <div className="flex items-center justify-between">
                        <span className="text-xs text-muted-foreground">{activity.time}</span>
                        <span className="text-xs font-semibold text-[#14F195]">{activity.amount}</span>
                      </div>
                    </div>
                  </motion.div>
                ))}
